...
Microsoft::WRL::ComPtr<ID3D12RootSignature> PipeLineManager::CreatePrimitiveRootSignature(Microsoft::WRL::ComPtr<ID3D12RootSignature> rootSignature)
{
	HRESULT hr;
	// RootSignature作成
	D3D12_ROOT_SIGNATURE_DESC descriptionRootSignature{};
...
Microsoft::WRL::ComPtr<ID3D12PipelineState> PipeLineManager::CreatePrimitiveGraphicsPipeLine(Microsoft::WRL::ComPtr<ID3D12PipelineState> graphicsPipelineState, Microsoft::WRL::ComPtr<ID3D12RootSignature> rootSignature)
{
	HRESULT hr;
	...
	rasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
	// Shaderをコンパイルする
	IDxcBlob* vertexShaderBlob = dxCommon_->CompileShader(L"./resources/shaders/Primitive.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob != nullptr);

	IDxcBlob* geometryShaderBlob = dxCommon_->CompileShader(L"./resources/shaders/BasicGeometryShader.VS.hlsl", L"vs_6_0");
	assert(geometryShaderBlob != nullptr);

	IDxcBlob* pixelShaderBlob = dxCommon_->CompileShader(L"./resources/shaders/Primitive.PS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
